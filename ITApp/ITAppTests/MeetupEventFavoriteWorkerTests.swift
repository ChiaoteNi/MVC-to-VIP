...
@Suite("MeetupEventFavoriteWorkerTests")
struct MeetupEventFavoriteWorkerTests {

    private let sut: MeetupEventFavoriteWorker = .shared

    @Test("Toggle Meetup Event Favorite State")
    func testToggleMeetupEventFavoriteState() async {
        ...
        #expect(
            sut.getFavoriteState(withID: meetupEventID) == .unfavorite,
            "default meetup event should be unfavorite."
        )
        persistenceWorker.verifyTestActionsOrder(to: [
        ...
        #expect(
            sut.getFavoriteState(withID: meetupEventID) == .favorite,
            "set favorite on, should be favorite."
        )
        persistenceWorker.verifyTestActionsOrder(to: [
            .fetchFavoriteState(meetupEventID: fullID),
            .setFavoriteState(isFavorite: true, meetupEventID: fullID),
        ...
        #expect(
            sut.getFavoriteState(withID: meetupEventID) == .unfavorite,
            "set favorite off, should be unfavorite."
        )
        persistenceWorker.verifyTestActionsOrder(to: [
            .fetchFavoriteState(meetupEventID: fullID),
            .setFavoriteState(isFavorite: true, meetupEventID: fullID),
...
private extension MeetupEventFavoriteWorkerTests {
    ...
    final class UserDefaultsMock: UserDefaults {
        ...
        func verifyTestActionsOrder(to actions: [TestAction]) {
            #expect(testActions == actions, "Test actions order should match.")
        }
